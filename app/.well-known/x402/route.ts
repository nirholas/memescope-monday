...
function toSmallestUnit(price: string): string {
  return (parseFloat(price) * 10 ** USDC_DECIMALS).toString()
}
...
export async function GET() {
  const accepts = Object.entries(X402_TIERS).map(([, tier]) => ({
    ...
    maxAmountRequired: toSmallestUnit(tier.price),
    resource: `/api/payment/x402/settle`,
    description: tier.description,
    mimeType: "application/json",
    payTo: X402_CONFIG.receiverAddress,
    maxTimeoutSeconds: tier.maxTimeoutSeconds,
