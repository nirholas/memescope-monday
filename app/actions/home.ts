...
export async function getTopVotedProjects(limit: number = PROJECT_LIMITS_VARIABLES.TODAY_LIMIT) {
...
export async function getNewlyListedProjects(limit: number = PROJECT_LIMITS_VARIABLES.TODAY_LIMIT) {
  const userId = await getCurrentUserId()
  const newlyListedBase = await db
    .select({
      id: projectTable.id,
      name: projectTable.name,
      slug: projectTable.slug,
  ...
  return enrichProjectsWithUserData(newlyListedBase, userId)
}
