...
export async function getRelatedCoins(projectId: string, categoryIds: string[], limit = 3) {
  ...
  const relatedProjects = await db
    ...
    .from(project)
    .innerJoin(projectToCategory, eq(project.id, projectToCategory.projectId))
    .leftJoin(upvote, eq(upvote.projectId, project.id))
    .where(
      and(
    ...
    .groupBy(project.id, project.name, project.slug, project.ticker, project.logoUrl, project.chain)
    .orderBy(sql`count(distinct ${upvote.id}) DESC`)
    .limit(limit)
